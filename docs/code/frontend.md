...
Assistant hiện có ba mode: chung, tư vấn bữa ăn và tham khảo sức khỏe. Conversation giữ nguyên mode; turn hiển thị có/không personalization, trạng thái grounding và danh sách nguồn. Profile là nơi bật/tắt consent bằng notice version hiện hành. Không suy ra personalization chỉ từ việc người dùng có profile: backend mới là nguồn quyết định quyền.

Shopping list toàn plan tạo view-model bằng `shoppingListView.ts`: các row cùng `item_kind + ingredient_id + unit` được gộp và giữ `sourceItemIds`. Một checkbox nhóm gọi bulk PATCH; trạng thái partial phải được thể hiện thay vì giả định tất cả source item giống nhau. Day view không gộp. `ShoppingListDocument` dùng chung cho print/public và hiện chỉ render phần “Cần mua”; ledger/carryover vẫn có trên màn owner tương tác.
