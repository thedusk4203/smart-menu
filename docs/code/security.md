...
Shopping share hết hạn sau 7 ngày, có thể scope theo ngày và revoke. Người có token còn hiệu lực có thể xem/toggle trạng thái purchased trong scope, nên token là capability nhạy cảm. Không đưa token thật vào docs, log, screenshot, analytics hoặc error message.

## Logging/retention

AI request log và conversation history khác nhau. Redact secret; hạn chế content nhạy cảm. Conversation inactive dọn sau 30 ngày; request logs retention 30 ngày theo policy hiện tại. Retention không thay thế quyền access hoặc encryption.
...
5. Chỉ hiển thị trạng thái cấu hình/masked metadata, test connection có redaction; đọc secret server-side từ encrypted config/env và không đưa vào response/log.
