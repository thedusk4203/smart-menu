...
interface FormIngredient { ingredient_id: number; name: string; quantity: string; unit: string; }
...
function toForm(item: AdminDish): DishForm {
  return { name: item.name, dish_type: item.dish_type, cooking_method: item.cooking_method || "", description: item.description || "", instructions: item.instructions || "", tags: item.tags.join(", "), is_active: item.is_active, ingredients: item.ingredients.map((ingredient) => ({ ingredient_id: ingredient.ingredient_id, name: ingredient.name, quantity: String(ingredient.quantity), unit: ingredient.unit })) };
}

function toPayload(form: DishForm): AdminDishWrite {
  return { name: form.name.trim(), dish_type: form.dish_type, cooking_method: form.cooking_method || null, description: form.description.trim() || null, instructions: form.instructions.trim() || null, tags: form.tags.split(",").map((tag) => tag.trim()).filter(Boolean), is_active: form.is_active, ingredients: form.ingredients.map((ingredient) => ({ ingredient_id: ingredient.ingredient_id, quantity: Number(ingredient.quantity), unit: ingredient.unit.trim() })) };
}
...
export function AdminDishes() {
  ...
  const addIngredient = (id: number, name: string, unit: string) => { if (form.ingredients.some((item) => item.ingredient_id === id)) return; setForm((current) => ({ ...current, ingredients: [...current.ingredients, { ingredient_id: id, name, quantity: "1", unit }] })); };
  ...
  return <div>
    <PageHeader title="Món thành phần" description="Công thức chuẩn mà planner dùng để ghép thành bữa/mâm món." actions={<div className="flex flex-wrap gap-2"><AdminExportDialog entityType="dishes" filteredParams={{ search: search.trim() || undefined, dish_type: dishType, status, quality }} filteredTotal={total} /><Button onClick={openCreate}><Plus className="h-4 w-4" /> Thêm món</Button></div>} />
    ...
    <Modal open={open} onClose={() => setOpen(false)} title={editing ? `Sửa ${editing.name}` : "Thêm món thành phần"} size="lg" footer={<>{editing && <Button type="button" variant="danger" onClick={() => setDeleting(editing)}><Trash2 className="h-4 w-4" /> Xóa món</Button>}<Button variant="ghost" onClick={() => setOpen(false)}>Hủy</Button><Button form="admin-dish-form" type="submit" loading={saving}>Lưu công thức</Button></>}><form id="admin-dish-form" onSubmit={save} className="space-y-6"><section><h3 className="font-semibold text-gray-900">Thông tin món</h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><TextField className="sm:col-span-2" label="Tên món" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /><SelectField label="Loại món" value={form.dish_type} options={DISH_TYPE_OPTIONS} onChange={(e) => setForm({ ...form, dish_type: e.target.value as DishType })} /><SelectField label="Cách chế biến" value={form.cooking_method} placeholder="Chưa xác định" options={METHOD_OPTIONS} onChange={(e) => setForm({ ...form, cooking_method: e.target.value as CookingMethod | "" })} /><TextField className="sm:col-span-2" label="Thẻ" value={form.tags} hint="Cách nhau bằng dấu phẩy." onChange={(e) => setForm({ ...form, tags: e.target.value })} /><Textarea className="sm:col-span-2" label="Mô tả" rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><Textarea className="sm:col-span-2" label="Hướng dẫn chế biến" rows={4} value={form.instructions} hint="Món chưa có hướng dẫn sẽ được đánh dấu cần bổ sung." onChange={(e) => setForm({ ...form, instructions: e.target.value })} /></div></section><section className="border-t border-sand-200 pt-5"><div className="flex items-baseline justify-between gap-4"><div><h3 className="font-semibold text-gray-900">Công thức</h3><p className="mt-1 text-sm text-gray-600">Giá và dinh dưỡng được tính tự động từ danh sách này.</p></div><span className="text-sm text-gray-600">{form.ingredients.length} nguyên liệu</span></div><div className="mt-3"><IngredientPicker placeholder="Tìm và thêm nguyên liệu" onSelect={(item) => addIngredient(item.id, item.name, item.default_unit)} /></div>{form.ingredients.length > 0 ? <ul className="mt-3 divide-y divide-sand-100 rounded-xl border border-sand-200">{form.ingredients.map((item) => <li key={item.ingredient_id} className="flex flex-wrap items-center gap-2 p-3"><p className="min-w-40 flex-1 font-medium text-gray-900">{item.name}</p><input aria-label={`Định lượng ${item.name}`} type="number" min="0.01" step="0.01" value={item.quantity} onChange={(e) => updateIngredient(item.ingredient_id, { quantity: e.target.value })} className="h-10 w-24 rounded-lg border border-sand-200 px-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-400" /><input aria-label={`Đơn vị ${item.name}`} value={item.unit} onChange={(e) => updateIngredient(item.ingredient_id, { unit: e.target.value })} className="h-10 w-20 rounded-lg border border-sand-200 px-2 text-sm focus:outline-none focus:ring-2 focus-visible:ring-2 focus:ring-brand-400" /><button type="button" onClick={() => removeIngredient(item.ingredient_id)} className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-red-700 transition hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400" aria-label={`Bỏ ${item.name}`}><Trash2 className="h-4 w-4" /></button></li>)}</ul> : <p className="mt-3 rounded-xl bg-sand-50 px-3 py-3 text-sm text-gray-600">Thêm ít nhất một nguyên liệu trước khi kích hoạt món.</p>}</section></form></Modal>
