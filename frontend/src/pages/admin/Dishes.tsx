...
export function AdminDishes() {
  ...
  const [loading, setLoading] = useState(true); const [error, setError] = useState<UserFeedback | null>(null);
  ...
  const load = useCallback(async () => { setLoading(true); setError(null); try { const page = await adminApi.dishes({ search: search.trim() || undefined, dish_type: dishType, status, quality, limit: LIMIT, offset }); setItems(page.items); setTotal(page.total); } catch (err) { setError(toUserFeedback(err, "admin_action", "admin")); } finally { setLoading(false); } }, [dishType, offset, quality, search, status]);
  ...
  const openEdit = async (item: AdminDish) => { try { const detail = await adminApi.dish(item.id); setEditing(detail); setForm(toForm(detail)); setOpen(true); } catch (err) { toast.error(adminFeedbackMessage(err)); } };
  ...
  const save = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { const payload = toPayload(form); const result = editing ? await adminApi.updateDish(editing.id, payload) : await adminApi.createDish(payload); setItems((current) => editing ? current.map((item) => item.id === result.id ? result : item) : [result, ...current]); if (!editing) setTotal((value) => value + 1); setOpen(false); toast.success(editing ? "Đã cập nhật công thức." : "Đã tạo món thành phần."); } catch (err) { toast.error(adminFeedbackMessage(err)); } finally { setSaving(false); } };
  const toggle = async (item: AdminDish) => { setSavingId(item.id); try { const result = await adminApi.setDishActive(item.id, !item.is_active); setItems((current) => current.map((value) => value.id === item.id ? result : value)); toast.success(result.is_active ? "Đã kích hoạt món." : "Đã ẩn món khỏi hệ thống lập thực đơn."); } catch (err) { toast.error(adminFeedbackMessage(err)); } finally { setSavingId(null); } };
  const deleteDish = async () => { if (!deleting) return; setDeletingId(deleting.id); try { await adminApi.deleteDish(deleting.id); setItems((current) => current.filter((item) => item.id !== deleting.id)); setTotal((value) => Math.max(0, value - 1)); setOpen(false); setEditing(null); setDeleting(null); toast.success("Đã xóa món thành phần."); } catch (err) { toast.error(adminFeedbackMessage(err)); } finally { setDeletingId(null); } };
  const downloadFlexSuggestions = async () => {
    ...
    try {
      ...
      toast.success("Đã tải tệp gợi ý. Hãy xem lại (review) trước khi nhập lại (import).");
    } catch (err) {
  ...
  return <div>
    <PageHeader title="Món thành phần" description="Công thức chuẩn mà hệ thống lập thực đơn (planner) dùng để ghép thành bữa hoặc mâm món." actions={<div className="flex flex-wrap gap-2"><Button variant="secondary" loading={downloadingSuggestions} onClick={() => void downloadFlexSuggestions()}><Download className="h-4 w-4" /> Gợi ý tận dụng phần dư</Button><AdminExportDialog entityType="dishes" filteredParams={{ search: search.trim() || undefined, dish_type: dishType, status, quality }} filteredTotal={total} /><Button onClick={openCreate}><Plus className="h-4 w-4" /> Thêm món</Button></div>} />
