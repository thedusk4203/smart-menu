...
import { Badge, Button, Card, Modal, NumberField, PageHeader, SelectField, TextField } from "../../components/ui";
...
import type { AdminIngredient, AdminIngredientWrite } from "../../types/admin";
...
export function AdminIngredients() {
  ...
  return (
    <div>
      <PageHeader title="Nguyên liệu" description="Quản lý đơn vị, dinh dưỡng, giá và dữ liệu đầu vào cho mọi công thức." actions={<Button onClick={openCreate}><Plus className="h-4 w-4" /> Thêm nguyên liệu</Button>} />
      <div className="mb-4 grid gap-3 rounded-2xl border border-sand-200 bg-white p-3 shadow-sm lg:grid-cols-[minmax(0,1fr)_10rem_10rem_13rem]">
        ...
        <SelectField value={quality} onChange={(e) => setQualityFilter(e.target.value)} placeholder="Tất cả chất lượng" options={[{ value: "missing_price", label: "Thiếu giá" }, { value: "missing_nutrition", label: "Thiếu dinh dưỡng" }, { value: "missing_conversion", label: "Cần quy đổi" }]} />
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? `Sửa ${editing.name}` : "Thêm nguyên liệu"} size="lg" footer={<><Button variant="ghost" onClick={() => setOpen(false)}>Hủy</Button><Button form="admin-ingredient-form" type="submit" loading={saving}>Lưu nguyên liệu</Button></>}>
        <form id="admin-ingredient-form" onSubmit={save} className="space-y-6">
          ...
          <section className="border-t border-sand-200 pt-5"><h3 className="font-semibold text-gray-900">Thêm mốc giá</h3><p className="mt-1 text-sm text-gray-600">Lưu một mốc mới, không ghi đè lịch sử giá trước đó.</p><div className="mt-3 grid gap-4 sm:grid-cols-2"><NumberField label="Giá gốc" min="0" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} /><TextField label="Đơn vị giá" value={form.price_unit} onChange={(e) => setForm({ ...form, price_unit: e.target.value })} /><NumberField label={`Giá / ${form.default_unit || "đơn vị"}`} min="0" value={form.price_per_default_unit} hint="Bắt buộc khi nhập giá gốc." onChange={(e) => setForm({ ...form, price_per_default_unit: e.target.value })} /><TextField label="Nguồn giá" value={form.price_source} placeholder="Chợ, siêu thị, khảo sát..." onChange={(e) => setForm({ ...form, price_source: e.target.value })} /></div></section>
        </form>
      </Modal>
    </div>
  );
