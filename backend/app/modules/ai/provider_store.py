    ...
    def list(self) -> list[dict[str, Any]]:
        rows = self.session.execute(
            text("""SELECT id, name, provider_type, base_url, model, api_key_suffix,
                           ...
                           native_web_search_enabled, capability_checked_at,
        ...
        if row is None:
            ...
            if count == 0 and settings.ai_enabled:
                return ProviderConfig(
                    ...
                    native_web_search_enabled=settings.ai_native_web_search_enabled,
                    source="environment",
                )
            return None
        data = dict(row)
        ...
        row = self.session.execute(
            text("""INSERT INTO llm_provider_configs
                    (name, provider_type, base_url, model, encrypted_api_key, api_key_suffix,
                     timeout_seconds, native_web_search_enabled, created_by, updated_by)
                    VALUES (:name, :ptype, :url, :model, :key, :suffix, :timeout, :native_web, :actor, :actor)
                    RETURNING id"""),
            {"name": data.name.strip(), "ptype": data.provider_type, "url": base_url,
             "model": data.model.strip(), "key": encrypted, "suffix": suffix,
             "timeout": data.timeout_seconds, "native_web": data.native_web_search_enabled,
             "actor": actor_id},
        ).scalar_one()
    ...
    def update(self, config_id: int, data: Any, actor_id: int) -> dict[str, Any]:
        ...
        self.session.execute(
            text("""UPDATE llm_provider_configs SET
                    ...
                    native_web_search_enabled=:native_web, capability_checked_at=NULL,
            ...
            {"id": config_id, "name": data.name.strip(), "ptype": data.provider_type,
             ...
             "suffix": suffix, "timeout": data.timeout_seconds,
             "native_web": data.native_web_search_enabled, "actor": actor_id},
        )
        ...
        new_id = self.session.execute(
            text("""INSERT INTO llm_provider_configs
                    ...
                    SELECT name || ' (bản sao)', provider_type, base_url, model,
                           encrypted_api_key, api_key_suffix, timeout_seconds,
                           native_web_search_enabled, :actor, :actor
    ...
    def mark_test(self, config_id: int, *, success: bool, mode: str | None, error: str | None) -> dict[str, Any]:
        self.session.execute(
            text("""UPDATE llm_provider_configs SET tested_version=config_version,
                    ...
                    last_tested_at=NOW(), capability_checked_at=NOW(),
...
class AIRequestLogStore:
    ...
    def write(self, **values: Any) -> None:
        if self.actor_id is not None:
            ...
            if values.get("user_id") != self.actor_id:
                raise AuthorizationError("Không thể ghi log AI cho người dùng khác.")
        self.session.execute(
