...
from app.modules.meal_planning.domain import ComposedMeal, DishCandidate, DishIngredientSnapshot, PlanRequest
...
def test_regenerate_changes_signature_without_worse_than_five_percent_nutrition_score():
    ...
    regenerated = DishPlanner().generate(
        request(previous_plan_signature=original.plan_data["plan_signature"]), candidates, seed=8
    )
...
def test_small_cpsat_instance_matches_brute_force_nutrition_optimum():
    ...
    result = DishPlanner().generate(plan_request, candidates, seed=4)
